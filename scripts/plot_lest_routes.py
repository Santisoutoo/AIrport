...
def main():
    ...
    for s in g.data.get("stands", []):
        ax.plot(
            ...
            "s", color="#888888", markersize=2.2, zorder=2,
        )
    ...
    if r1.get("success"):
        plot_route(ax, g.graph, r1["path_node_ids"], "#2e7d32",
                   "Stand 11 -> RWY 35 (directa)")

    # Route 2: stand 11 -> runway 35 forcing via E1 (shows backtracking)
    r2 = g.find_route_from_position(
        stand11["latitude"], stand11["longitude"], "35", via=["E1"]
    )
    if r2.get("success"):
        plot_route(ax, g.graph, r2["path_node_ids"], "#c62828",
                   "Stand 11 -> RWY 35 via E1", linewidth=1.8)

    # Route 3: free position near stand 19 -> runway 17
    stand19 = next(s for s in g.data["stands"] if s["stand_id"] == "19")
    r3 = g.find_route_from_position(
        stand19["latitude"], stand19["longitude"], "17"
    )
    if r3.get("success"):
        plot_route(ax, g.graph, r3["path_node_ids"], "#1565c0",
                   "Stand 19 -> RWY 17")

    # Mark route start points
    ax.plot(stand11["longitude"], stand11["latitude"], "o",
            color="#2e7d32", markersize=8, zorder=5)
    ax.plot(stand19["longitude"], stand19["latitude"], "o",
            color="#1565c0", markersize=8, zorder=5)

    ax.set_xlabel("Longitud (grados)")
    ax.set_ylabel("Latitud (grados)")
    ax.set_title(
        "Aerodromo de Santiago (LEST): grafo de rodaje y tres rutas de prueba"
    )
    ...
    print(f"Routes: r1={r1.get('success')} r2={r2.get('success')} "
          f"r3={r3.get('success')}")
    if r1.get("success"):
        print(f"  r1: {len(r1['path_node_ids'])} nodes, "
              f"{r1['total_distance_m']} m, {r1['taxiway_sequence']}")
    if r2.get("success"):
        print(f"  r2: {len(r2['path_node_ids'])} nodes, "
              f"{r2['total_distance_m']} m, {r2['taxiway_sequence']}")
    if r3.get("success"):
        print(f"  r3: {len(r3['path_node_ids'])} nodes, "
              f"{r3['total_distance_m']} m, {r3['taxiway_sequence']}")
