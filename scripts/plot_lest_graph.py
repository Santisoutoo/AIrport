...
def main():
    ...
    for nid in g.graph.nodes():
        n = g.graph.nodes[nid]
        if nid in main_cc:
            ax.plot(n["lon"], n["lat"], "o", color="#1565c0", markersize=2.5, zorder=2)
        else:
            ax.plot(n["lon"], n["lat"], "o", color="#bfc8d2", markersize=2.0, zorder=2)

    # Stands
    for s in g.data.get("stands", []):
        ax.plot(s["longitude"], s["latitude"], "s", color="#666666", markersize=2.5, zorder=3)
    ...
    legend_items = [
        Line2D([0], [0], marker="^", color="w", markerfacecolor="#0d3b66", markersize=9, label="Umbral de pista"),
        Line2D(
            [0],
            [0],
            marker="o",
        ...
        Line2D([0], [0], marker="o", color="w", markerfacecolor="#bfc8d2", markersize=7, label="Nodo huerfano"),
        Line2D([0], [0], marker="s", color="w", markerfacecolor="#666666", markersize=7, label="Estacionamiento"),
    ...
    plt.close(fig)
    print(f"Saved: {OUT}")
    print(f"Stats: {g.graph.number_of_nodes()} nodes, {g.graph.number_of_edges()} edges, main_cc={len(main_cc)}")
