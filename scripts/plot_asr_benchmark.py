...
def discover_configs():
    ...
    for p in sorted(BENCH.glob("asr_*_*.csv")):
        ...
        out.append({"model": model, "tier": tier, "ctx": ctx, "rows": rows,
                    "fields": fields, "label": config_label(model, tier),
...
def agg(rows):
    ...
    for scope in ["del", "gnd", "twr", "ALL"]:
        ...
        out[scope] = {"n": len(s), "wer": (S + D + I) / W if W else 0.0,
                      "S": S, "D": D, "I": I, "p50": p50}
    return out
...
PA = [{"id": r["audio_id"], "dep": r["dep"], "wer": num(r["wer_raw"]),
       "dur": num(r["duration_s"]), "adur": num(r["audio_dur_s"])}
      for r in med_rows] if med_rows else []
...
def table_summary():
    if not M:
        print("[skip] table_summary: no medium c2 generic CSV"); return
    ...
    for d in ["del", "gnd", "twr", "ALL"]:
        a = M[d]
        rows.append([DEPNAME[d], a["n"], f"{a['wer']*100:.1f}%",
                     a["S"], a["D"], a["I"], f"{a['p50']:.0f} s"])
    ...
    for (r, c), cell in tbl.get_celld().items():
        ...
        if r == 0:
            cell.set_facecolor("#f4f4f0"); cell.set_text_props(color=INK2, fontweight="bold")
        else:
            if c == 0:
    ...
    ax.set_title("Medium — word error rate by controller position",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=14)
    fig.text(0.5, 0.02, "60 synthesized clips · raw transcription vs reference · CPU 2 vCPU",
...
def table_compare():
    if not M or not T:
        print("[skip] table_compare: needs medium + tiny c2 generic CSVs"); return
    fig, ax = plt.subplots(figsize=(9.6, 1.9))
    ax.axis("off")
    cols = ["Model", "Params", "WER (all)", "Subs", "Del", "Ins", "Verdict"]
    rows = [
        ["tiny", "~38 M", f"{T['ALL']['wer']*100:.0f}%", T['ALL']['S'], T['ALL']['D'],
         T['ALL']['I'], "Unusable (loops)"],
        ["medium", "~770 M", f"{M['ALL']['wer']*100:.1f}%", M['ALL']['S'], M['ALL']['D'],
         M['ALL']['I'], "Usable"],
    ]
    tbl = ax.table(cellText=rows, colLabels=cols, cellLoc="center", loc="center",
    ...
    for (r, c), cell in tbl.get_celld().items():
        ...
        if r == 0:
            ...
            cell.set_text_props(color=CRIT if r == 1 else "#0a8a0a", fontweight="bold")
    ax.set_title("tiny vs medium (jlvdoorn atco2) — same 60 audios",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=14)
...
def chart_wer():
    if not M:
        print("[skip] chart_wer: no medium c2 generic CSV"); return
    ...
    for y, v in zip(ys, vals):
        ax.text(v + 0.3, y, f"{v:.1f}%", va="center", ha="left",
                fontsize=11, fontweight="bold", color=INK)
    ax.set_yticks(list(ys)); ax.set_yticklabels([DEPNAME[d] for d in order], fontweight="bold")
    ax.invert_yaxis()
    ax.set_xlim(0, 16); ax.set_xlabel("Word error rate (%)")
    ax.xaxis.grid(True, color=GRID, zorder=0); ax.set_axisbelow(True)
    for s in ["top", "right", "left"]:
        ax.spines[s].set_visible(False)
    ax.set_title("Medium — WER by controller position",
...
def chart_sdi():
    if not M:
        print("[skip] chart_sdi: no medium c2 generic CSV"); return
    fig, ax = plt.subplots(figsize=(7.2, 3.0))
    order = ["del", "gnd", "twr", "ALL"]
    ys = range(len(order))
    left = [0] * len(order)
    for key in ["S", "D", "I"]:
        ...
        ax.barh(ys, vals, left=left, color=SDI_COL[key], height=0.62,
    ...
    ax.set_yticks(list(ys)); ax.set_yticklabels([DEPNAME[d] for d in order], fontweight="bold")
    ax.invert_yaxis(); ax.set_xlim(0, 120); ax.set_xlabel("Word errors (count)")
    ax.xaxis.grid(True, color=GRID, zorder=0); ax.set_axisbelow(True)
    for s in ["top", "right", "left"]:
        ax.spines[s].set_visible(False)
    ax.legend(loc="upper right", frameon=False, fontsize=9.5)
    ax.set_title("Medium — where the errors come from (S / D / I)",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
...
def chart_dist():
    if not PA:
        print("[skip] chart_dist: no medium c2 generic CSV"); return
    ...
    ax.set_xlim(-1, len(data)); ax.set_ylim(0, 55)
    ax.set_ylabel("Word error rate (%)"); ax.set_xticks([])
    ...
    ax.text((n_perfect - 1) / 2, 3.6, f"{n_perfect} perfect (0%)", color=MUTED,
            fontsize=8.5, ha="center", style="italic")
    worst = data[-1]
    ax.annotate(f"{worst['id']}  {worst['wer']*100:.0f}%",
                xy=(len(data) - 1, worst["wer"] * 100),
                xytext=(len(data) - 4, 45), ha="right", fontsize=9, color=INK2,
                arrowprops=dict(arrowstyle="->", color=MUTED, lw=1))
    ax.legend(handles=[Patch(color=DEP_COL[d], label=DEPNAME[d]) for d in ["del", "gnd", "twr"]],
              loc="upper left", frameon=False, fontsize=9.5)
    ax.set_title("Medium — every utterance, sorted best to worst (60 audios)",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
...
def chart_rtf():
    if not PA:
        print("[skip] chart_rtf: no medium c2 generic CSV"); return
    fig, ax = plt.subplots(figsize=(8.4, 3.6))
    for d in ["del", "gnd", "twr"]:
        pts = [p for p in PA if p["dep"] == d]
        ax.scatter([p["adur"] for p in pts], [p["dur"] for p in pts],
                   s=44, color=DEP_COL[d], edgecolor="white", linewidth=0.8,
                   alpha=0.9, zorder=3, label=DEPNAME[d])
    ...
    ax.set_xlim(0, 16); ax.set_ylim(0, 100)
    ax.set_xlabel("Audio length (s)"); ax.set_ylabel("Server inference time (s)")
    ax.grid(True, color=GRID, zorder=0); ax.set_axisbelow(True)
    for s in ["top", "right"]:
        ax.spines[s].set_visible(False)
    ax.legend(loc="lower right", frameon=False, fontsize=9.5)
    ax.set_title("Medium on CPU — inference time vs clip length (RTF ≈ 9×)",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
...
def chart_proc_time():
    if not PA:
        print("[skip] chart_proc_time: no medium c2 generic CSV"); return
    fig, ax = plt.subplots(figsize=(9.6, 3.2))
    ...
    ax.text(len(data) - 1, med + 1.5, f"median {med:.0f} s", color=INK2, fontsize=9.5,
            va="bottom", ha="right", fontweight="bold")
    ax.set_xlim(-1, len(data)); ax.set_ylim(0, top)
    ax.set_ylabel("Server processing time (s)"); ax.set_xticks([])
    ax.yaxis.grid(True, color=GRID, zorder=0); ax.set_axisbelow(True)
    for s in ["top", "right", "bottom"]:
        ax.spines[s].set_visible(False)
    ...
    ax.legend(handles=[Patch(color=DEP_COL[d], label=DEPNAME[d]) for d in ["del", "gnd", "twr"]],
              loc="upper left", frameon=False, fontsize=9.5)
    ax.set_title("Medium on CPU — processing time per audio (60 clips, sorted)",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
...
def chart_proc_compare():
    if not PA or not tin_rows:
        print("[skip] chart_proc_compare: needs medium + tiny c2 generic CSVs"); return
    fig, ax = plt.subplots(figsize=(8.0, 3.4))
    tin_pa = [{"dep": r["dep"], "dur": num(r["duration_s"])}
              for r in tin_rows if num(r["duration_s"]) is not None]
    series = [("tiny", tin_pa, "~38 M"), ("medium", PA, "~770 M")]
    import random
    random.seed(1)
    for i, (name, pa, _) in enumerate(series):
        durs = [p["dur"] for p in pa]
        bp = ax.boxplot([durs], positions=[i], widths=0.5, vert=True,
                        patch_artist=True, showfliers=False, zorder=2,
                        medianprops=dict(color=INK, lw=1.6),
                        boxprops=dict(facecolor="#f4f4f0", edgecolor="#cdccc4"),
                        whiskerprops=dict(color="#cdccc4"), capprops=dict(color="#cdccc4"))
        for p in pa:
            ax.scatter(i + (random.random() - 0.5) * 0.34, p["dur"], s=26,
                       color=DEP_COL[p["dep"]], edgecolor="white", linewidth=0.6,
                       alpha=0.85, zorder=3)
        med = sorted(durs)[len(durs) // 2]
        ax.text(i, med, f"  {med:.0f} s", va="center", ha="left", fontsize=10,
                fontweight="bold", color=INK)
    ax.set_xticks([0, 1]); ax.set_xticklabels(["tiny\n~38 M", "medium\n~770 M"], fontweight="bold")
    ax.set_ylim(0, 100); ax.set_ylabel("Server processing time (s)")
    ax.yaxis.grid(True, color=GRID, zorder=0); ax.set_axisbelow(True)
    for s in ["top", "right", "bottom"]:
        ax.spines[s].set_visible(False)
    ax.legend(handles=[Patch(color=DEP_COL[d], label=DEPNAME[d]) for d in ["del", "gnd", "twr"]],
              loc="upper left", frameon=False, fontsize=9)
    ax.text(0.5, 0.34, "tiny ~3× faster —\nbut loops (unusable)", transform=ax.transAxes,
            color=MUTED, fontsize=9.5, va="center", ha="center", style="italic")
    ax.set_title("Processing time per audio — tiny vs medium (2 vCPU)",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
...
def chart_latency_by_config():
    ...
    if not items:
        print("[skip] chart_latency_by_config: no configs with duration data"); return
    items.sort(key=lambda c: -median([num(r["duration_s"]) for r in c["rows"]
                                      if num(r["duration_s"]) is not None]))
    import random
    random.seed(3)
    ...
    for i, c in enumerate(items):
        ...
        ax.boxplot([durs], positions=[i], widths=0.5, vert=True, patch_artist=True,
                   showfliers=False, zorder=2, medianprops=dict(color=INK, lw=1.6),
        ...
        for r in c["rows"]:
            ...
            if d is None:
                continue
            ax.scatter(i + (random.random() - 0.5) * 0.34, d, s=24,
                       color=DEP_COL.get(r["dep"], "#8a8a86"), edgecolor="white",
    ...
    ax.set_ylabel("Server processing time (s)"); ax.set_ylim(0, None)
    ax.yaxis.grid(True, color=GRID, zorder=0); ax.set_axisbelow(True)
    for s in ["top", "right", "bottom"]:
        ax.spines[s].set_visible(False)
    ax.legend(handles=[Patch(color=DEP_COL[d], label=DEPNAME[d]) for d in ["del", "gnd", "twr"]],
              loc="upper right", frameon=False, fontsize=9)
    ax.set_title("Latency by configuration — server processing time per audio",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
...
def chart_wer_context():
    ...
    if not groups:
        print("[skip] chart_wer_context: no usable configs (wer_raw)"); return
    fig, ax = plt.subplots(figsize=(max(6.0, 1.8 * len(groups) + 2.0), 3.4))
    xs = range(len(groups)); bw = 0.38
    b1 = ax.bar([x - bw / 2 for x in xs], [g[1] for g in groups], bw,
                color="#2a78d6", zorder=3, label="WER raw")
    b2 = ax.bar([x + bw / 2 for x in xs], [g[2] for g in groups], bw,
                color="#9ec5f4", zorder=3, label="WER post")
    for bars in (b1, b2):
        for b in bars:
            ax.text(b.get_x() + b.get_width() / 2, b.get_height() + 0.25,
                    f"{b.get_height():.1f}", ha="center", va="bottom", fontsize=9, color=INK2)
    ...
    for s in ["top", "right"]:
        ax.spines[s].set_visible(False)
    ax.legend(loc="upper right", frameon=False, fontsize=9.5)
    ax.set_title("WER by configuration and context — raw vs post-processed",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
...
def chart_entity_accuracy():
    ...
    if not groups:
        print("[skip] chart_entity_accuracy: no CSV has entity columns yet"); return
    data = []
    for c in groups:
    ...
    xs = range(len(data)); bw = 0.38
    b1 = ax.bar([x - bw / 2 for x in xs], [d[1] for d in data], bw,
                color="#184f95", zorder=3, label="Callsign")
    b2 = ax.bar([x + bw / 2 for x in xs], [d[2] for d in data], bw,
                color="#6da7ec", zorder=3, label="SID")
    for bars in (b1, b2):
        for b in bars:
            ax.text(b.get_x() + b.get_width() / 2, b.get_height() + 1.0,
                    f"{b.get_height():.0f}%", ha="center", va="bottom", fontsize=9, color=INK2)
    ax.set_xticks(list(xs))
    ax.set_xticklabels([f"{d[0]['label']}\n{d[0]['ctx']}" for d in data], fontweight="bold")
    ax.set_ylabel("Entity recovered (%)"); ax.set_ylim(0, 108)
    ax.yaxis.grid(True, color=GRID, zorder=0); ax.set_axisbelow(True)
    for s in ["top", "right"]:
        ax.spines[s].set_visible(False)
    ax.legend(loc="upper right", frameon=False, fontsize=9.5)
    ax.set_title("Entity recovery — callsign & SID accuracy by configuration",
                 fontsize=13, fontweight="bold", color=INK, loc="left", pad=10)
