...
def main():
    ...
    w = 0.35
    b1 = ax_pct.bar(x - w/2, PCT_SCHEMA,   w, color=COL_SCHEMA,
                    label="% schema valido")
    b2 = ax_pct.bar(x + w/2, PCT_COMPLETE, w, color=COL_COMPLETE,
    ...
    for bars in (b1, b2):
        for bar in bars:
            h = bar.get_height()
            ax_pct.annotate(f"{h:.1f}", xy=(bar.get_x() + bar.get_width() / 2, h),
    ...
    for bars in (b3, b4):
        for bar in bars:
            h = bar.get_height()
            ax_lat.annotate(f"{h:.1f}", xy=(bar.get_x() + bar.get_width() / 2, h),
                            xytext=(0, 3), textcoords="offset points",
                            ha="center", fontsize=8)
