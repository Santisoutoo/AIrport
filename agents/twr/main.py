...
async def lifespan(app: FastAPI):
    global runner
    runner = build_runner()
    print(f"[twr_agent] mode={config.AGENT_MODE} model={config.get_litellm_model()}")
    yield


app = FastAPI(title="TWR Agent", version="0.1.0", lifespan=lifespan)
...
async def agent_card():
    return {
        "name": "TWR",
        "description": "Tower controller — issues takeoff clearances, landing clearances, and go-around instructions",
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": "TWR", "mode": config.AGENT_MODE, "model": config.get_litellm_model()}
