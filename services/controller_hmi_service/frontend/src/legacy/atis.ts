...
import { setILSForArrivalRunway } from '../smr/render';
...
export function initAtisModal(): void {
  document.getElementById('atis-btn')?.addEventListener('click', open);
  document.getElementById('atis-overlay')?.addEventListener('click', onOverlayClick);
  document.querySelector('.atis-close-btn')?.addEventListener('click', close);
  document.querySelector('.atis-btn-cancel')?.addEventListener('click', close);
  document.querySelector('.atis-btn-send')?.addEventListener('click', send);
}

document.addEventListener('DOMContentLoaded', initAtisModal);

export { open as openAtisModal, close as closeAtisModal, send as sendAtis };
