...
(function () {
    ...
    const C_SWEEP_L = 'rgba(0,229,255,0.00)';
    const C_SWEEP_H = 'rgba(0,229,255,0.15)';
    const C_APT     = 'rgba(0,229,255,0.22)';
    const C_APT_DIM = 'rgba(0,229,255,0.09)';
    const C_ILS     = 'rgba(0,229,255,0.10)';
    ...
    let csIdx = 0;
    function nextCS() { return CS[csIdx++ % CS.length]; }
    ...
    // ── Draw: airport ─────────────────────────────────────────────────────
    function drawRunway(rw, fillCol, edgeCol, lineCol) {
        ctx.save();
        ctx.translate(CX, CY);
        ctx.rotate(rw.hdg);
        ...
        ctx.setLineDash([7, 5]);
        ctx.beginPath();
        ctx.moveTo(0, -rw.len); ctx.lineTo(0, rw.len);
        ctx.stroke();
        ctx.setLineDash([]);
        ...
        for (const end of [-rw.len, rw.len]) {
            ctx.beginPath();
            ctx.moveTo(-rw.w, end); ctx.lineTo(rw.w, end);
            ctx.stroke();
        }
    ...
    function drawAirport() {
        ...
        for (const sign of [-1, 1]) {
            ...
            ctx.beginPath();
            ctx.moveTo(tx, ty);
            ctx.lineTo(tx + Math.sin(RW_ARR.hdg) * maxR * 0.40 * sign,
                       ty - Math.cos(RW_ARR.hdg) * maxR * 0.40 * sign);
            ctx.stroke();
            // Funnel (±3.5°)
            for (const side of [-1, 1]) {
                ...
                ctx.beginPath();
                ctx.moveTo(tx, ty);
                ctx.lineTo(tx + Math.sin(fa) * maxR * 0.38 * sign,
                           ty - Math.cos(fa) * maxR * 0.38 * sign);
                ctx.stroke();
            }
        }
        ctx.setLineDash([]);
    ...
    function drawSweep(angle) {
        ...
        ctx.beginPath();
        ctx.moveTo(CX, CY);
        ctx.lineTo(CX + Math.cos(angle) * maxR * 1.1,
                   CY + Math.sin(angle) * maxR * 1.1);
        ctx.stroke();
    }

    // ── Draw & update: aircraft ───────────────────────────────────────────
    function updateDrawAircraft(dt) {
        for (const ac of aircraft) {
            ...
            if (ac.lit > 0) {
                ...
                ctx.lineWidth = 0.9;
                const hx = ac.vx, hy = ac.vy;
                const hlen = Math.sqrt(hx * hx + hy * hy) || 1;
                const nx = hx / hlen * 6, ny = hy / hlen * 6;
                const px = -ny * 0.45, py = nx * 0.45;
                ctx.beginPath();
    ...
    function drawTicks() {
        ...
        for (let deg = 0; deg < 360; deg += 10) {
            ...
            const r1 = maxR * 0.95, r2 = maxR * 1.0;
            ctx.beginPath();
